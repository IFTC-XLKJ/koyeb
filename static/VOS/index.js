        ...
        appBackstage.addEventListener("load", async () => {
            ...
            appBackstage.contentWindow.API = {
                File: class {
                    ...
                    async get() {
                        checkSystem(AppWindow.API.system, AppWindow.API.appid, this.path);
                        return await db.files.get({ name: this.path });
                    }
                    toFile(data, type) {
                        const name = this.path.split("/").pop();
                        ...
                        const handleMouseMove = (event) => {
                            if (isDragging) {
                                ...
                                if (appWindow && !moving) {
                                    ...
                                    const { left, top, width, height } = appWindow.getBoundingClientRect();
                                    const x = left + movementX;
                                    const y = top + movementY;
                                    appWindow.style.left = x <= 0 ? 0 : (x >= innerWidth - width ? innerWidth - width : x) + "px";
                                    appWindow.style.top = y <= 0 ? 0 : (y >= innerHeight - 50 - height ? innerHeight - 50 - height : y) + "px";
                                    moving = false;
                                }
                            }
                        };
